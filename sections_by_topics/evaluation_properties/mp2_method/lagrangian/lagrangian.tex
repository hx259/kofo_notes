...
\begin{align}
    \sum_{ij}\pdv{h _{ij}}{\kappa _{em}}\gamma _{ij}\Big|_{{\lambda=0}} =& \sum_{ij}(\cancel{h _{jm}\delta _{ie}}-h _{ie}\delta _{jm})\gamma _{ij} \nonumber \\
    =& -\sum_{i}h _{ie}\gamma _{im}
\end{align}
\begin{align}
    \sum_{ab}\pdv{h _{ab}}{\kappa _{em}}\gamma _{ab}\Big|_{{\lambda=0}} =& \sum_{ab}(h _{bm}\delta _{ae}-\cancel{h _{ae}\delta _{bm}})\gamma _{ab} \nonumber \\
    =& \sum_{b}h _{bm}\gamma _{eb} = \sum_{a}h _{am}\gamma _{ea}
\end{align}
\begin{align}
    \sum_{ijab}\pdv{\langle ab||ij\rangle}{\kappa _{em}}\Gamma _{ij}^{ab}\Big|_{{\lambda=0}} =& \sum_{ijab}\Big(\langle mb||ij\rangle \delta _{ae}+\langle am||ij\rangle \delta _{be}-\langle ab||ej\rangle \delta _{im}-\langle ab||ie\rangle \delta _{jm}\Big)\Gamma _{ij}^{ab} \nonumber \\
    =& \sum_{ijb}\langle mb||ij\rangle \Gamma _{ij}^{eb} + \sum_{ija}\langle am||ij\rangle \Gamma _{ij}^{ae} - \sum_{jab}\langle ab||ej\rangle \Gamma _{mj}^{ab} - \sum_{iab}\langle ab||ie\rangle \Gamma _{im}^{ab} \nonumber \\
    =& \sum_{ija}\langle ma||ij\rangle \Gamma _{ij}^{ea} + \sum_{ija}\langle am||ij\rangle \Gamma _{ij}^{ae} - \sum_{iab}\langle ab||ei\rangle \Gamma _{mi}^{ab} - \sum_{iab}\langle ab||ie\rangle \Gamma _{im}^{ab} \nonumber \\
    =& \sum_{ija}\langle am||ji\rangle \Gamma _{ij}^{ea} + \sum_{ija}\langle am||ji\rangle \Gamma _{ji}^{ae} - \sum_{iab}\langle ab||ei\rangle \Gamma _{mi}^{ab} - \sum_{iab}\langle ab||ei\rangle \Gamma _{im}^{ba} \nonumber \\
    =& 2\sum_{ija}\langle am||ji\rangle \Gamma _{ij}^{ea} - 2 \sum_{iab}\langle ab||ei\rangle \Gamma _{mi}^{ab}
\end{align}
in which the symmetry of MP2 amplitudes is exploited:
\begin{equation}
    T _{ij}^{ab} = T _{ji}^{ba} \Leftrightarrow \Gamma _{ij}^{ab} = \Gamma _{ji}^{ba}
\end{equation}  
Then:
\begin{align}
